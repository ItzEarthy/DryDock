...
The environmental panel shows the latest readings from the AM2320 sensors. The system now evaluates desiccant and enclosure health using the absolute internal humidity reading:

- **Inside Temperature and Humidity:** From AM2320 #1, placed inside the dry box. The dashboard treats the internal humidity (`hum_1`) as the primary indicator for desiccant health.
- **Ambient Temperature and Humidity:** From AM2320 #2, measuring the room conditions outside the dry box.

When the internal humidity exceeds the configured danger threshold, the dashboard will mark the silica as saturated and show an alert. A predictive warning may also appear when recent trends indicate the internal humidity is likely to reach the threshold within the configured predictive window.

Chart anomalies are now flagged for data points where the internal humidity (`hum_1`) exceeded the danger threshold.
...
Charts display humidity (both inside and ambient), temperature (both sensors), and weight over time. Points where the internal humidity (`hum_1`) exceeded the danger threshold are highlighted as anomalies.
